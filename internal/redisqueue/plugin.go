...
func (p *usageQueuePlugin) HandleUsage(ctx context.Context, record coreusage.Record) {
	...
	detail := requestDetail{
		Timestamp:    timestamp,
		FirstTokenMs: durationMilliseconds(record.FirstTokenLatency),
	...
	// The persistent history intentionally excludes the upstream API key. The
	// short-lived queue keeps its legacy payload for RESP consumers, while the
	// management usage page reads only this durable, redacted record set. The
	// durable record is written even when the optional short-lived queue is off.
	historyFail := fail
	...
	_ = usagehistory.Append(usagehistory.Record{
		...
		Failed:    failed,
		Fail:      usagehistory.Failure{StatusCode: historyFail.StatusCode, Body: historyFail.Body},
		Provider:  provider,
		Model:     modelName,
		Alias:     aliasName,
	...
	if !UsageStatisticsEnabled() || !Enabled() {
		return
	}
