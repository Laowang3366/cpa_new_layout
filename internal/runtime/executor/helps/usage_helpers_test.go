...
func TestUsageReporterBuildRecordIncludesReasoningEffort(t *testing.T) {
	ctx := usage.WithReasoningEffortFromRequest(context.Background(), []byte(`{"reasoning":{"effort":"high"}}`), "gpt-5")
	...
	if record.ReasoningEffort != "high" {
		t.Fatalf("ReasoningEffort = %q, want high", record.ReasoningEffort)
	}
}
...
func TestUsageReporterBuildRecordIncludesFirstTokenLatency(t *testing.T) {
	gin.SetMode(gin.TestMode)
	ginCtx, _ := gin.CreateTestContext(httptest.NewRecorder())
	timing := logging.NewResponseTiming()
	requestedAt := time.Now().Add(-500 * time.Millisecond)
	timing.MarkFirstChunk(requestedAt.Add(180 * time.Millisecond))
	ginCtx.Set(logging.ResponseTimingGinKey, timing)
	ctx := context.WithValue(context.Background(), "gin", ginCtx)
	reporter := &UsageReporter{requestedAt: requestedAt}

	record := reporter.buildRecord(ctx, usage.Detail{}, false)
	if record.FirstTokenLatency != 180*time.Millisecond {
		t.Fatalf("first token latency = %v, want 180ms", record.FirstTokenLatency)
	}
}
