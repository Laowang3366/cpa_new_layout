...
func (r *UsageReporter) buildRecordForModel(ctx context.Context, model string, detail usage.Detail, failed bool, fail usage.Failure) usage.Record {
	...
	return usage.Record{
		...
		FirstTokenLatency: r.firstTokenLatency(ctx),
		Latency:           r.latency(),
		Failed:            failed,
		Fail:              fail,
		Detail:            detail,
...
func (r *UsageReporter) firstTokenLatency(ctx context.Context) time.Duration {
	...
	raw, exists := ginCtx.Get(logging.ResponseTimingGinKey)
	if !exists {
		return 0
	}
	timing, ok := raw.(*logging.ResponseTiming)
	if !ok || timing == nil {
		return 0
	}
	latency := timing.FirstChunkTimestamp().Sub(r.requestedAt)
	if latency <= 0 {
		return 0
	}
	return latency
