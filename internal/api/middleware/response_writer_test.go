...
func TestStreamingWriteCapturesFirstChunkWhenRequestLoggingDisabled(t *testing.T) {
	gin.SetMode(gin.TestMode)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	...
	if _, err := wrapper.Write([]byte("data: first\n\n")); err != nil {
		t.Fatalf("write first chunk: %v", err)
	}
	first := timing.FirstChunkTimestamp()
	if first.IsZero() {
		t.Fatal("expected first chunk timestamp")
	}
	if _, err := wrapper.Write([]byte("data: second\n\n")); err != nil {
		t.Fatalf("write second chunk: %v", err)
	}
	if got := timing.FirstChunkTimestamp(); !got.Equal(first) {
		t.Fatalf("first chunk timestamp changed from %v to %v", first, got)
	}
}
