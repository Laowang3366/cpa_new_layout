...
type ResponseWriterWrapper struct {
	...
	responseTiming      *logging.ResponseTiming    // responseTiming shares first-chunk timing with usage reporting.
...
func NewResponseWriterWrapper(w gin.ResponseWriter, logger logging.RequestLogger, requestInfo *RequestInfo, responseTiming *logging.ResponseTiming) *ResponseWriterWrapper {
...
func (w *ResponseWriterWrapper) markFirstChunkTimestamp() {
	if w == nil || !w.firstChunkTimestamp.IsZero() {
		return
	}
	timestamp := time.Now()
	w.firstChunkTimestamp = timestamp
	if w.responseTiming != nil {
		w.responseTiming.MarkFirstChunk(timestamp)
	}
}

func (w *ResponseWriterWrapper) ensureStreamingDetected() {
	if w == nil || w.isStreaming {
		return
	}
	w.isStreaming = w.detectStreaming(w.ResponseWriter.Header().Get("Content-Type"))
}
