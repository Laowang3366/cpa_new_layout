...
// GetUsageRecords returns durable usage records without consuming the queue.
...
func parseUsageHistoryFilter(c *gin.Context) (usagehistory.Filter, error) {
	...
	start, err := parseUsageHistoryTime(firstNonEmptyUsage(c.Query("start"), c.Query("start_date")), false)
	if err != nil {
		return usagehistory.Filter{}, errors.New("invalid start time")
	}
	end, err := parseUsageHistoryTime(firstNonEmptyUsage(c.Query("end"), c.Query("end_date")), true)
	if err != nil {
	...
	return usagehistory.Filter{
		Start: start, End: end,
		Model: strings.TrimSpace(c.Query("model")), Provider: strings.TrimSpace(c.Query("provider")),
		Endpoint: strings.TrimSpace(c.Query("endpoint")), Source: strings.TrimSpace(c.Query("source")),
		AuthIndex: strings.TrimSpace(c.Query("auth_index")), Status: status,
		Search: strings.TrimSpace(c.Query("search")), SortBy: strings.TrimSpace(c.DefaultQuery("sort_by", "timestamp")),
		SortOrder: strings.TrimSpace(c.DefaultQuery("sort_order", "desc")), Granularity: granularity,
...
func firstNonEmptyUsage(values ...string) string {
