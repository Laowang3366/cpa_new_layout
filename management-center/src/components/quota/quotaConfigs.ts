...
import type { GuardianQuotaSnapshot } from '@/services/api';
...
export function hydrateGuardianSnapshots(
  files: AuthFileItem[],
  snapshots: GuardianQuotaSnapshot[],
  t: TFunction
): HydratedQuotaStates {
  const byName = new Map<string, GuardianQuotaSnapshot[]>();
  snapshots.forEach((snapshot) => {
    const current = byName.get(snapshot.name) ?? [];
    current.push(snapshot);
    byName.set(snapshot.name, current);
  });

  const result: HydratedQuotaStates = { claude: {}, codex: {}, kimi: {}, xai: {} };
  files.forEach((file) => {
    const entries = byName.get(file.name) ?? [];
    if (entries.length === 0) return;
    const provider = String(file.provider ?? file.type ?? '').toLowerCase();
    const successful = entries.filter((entry) => entry.status >= 200 && entry.status < 300);
    const firstError = entries.find((entry) => entry.status < 200 || entry.status >= 300);

    try {
      if (provider === 'codex') {
        const usage = successful.find((entry) => entry.endpoint === 'usage');
        const payload = usage ? parseCodexUsagePayload(usage.body) : null;
        if (!payload) throw new Error(firstError?.body || t('codex_quota.empty_windows'));
        const resetCredits = payload.rate_limit_reset_credits ?? payload.rateLimitResetCredits ?? null;
        result.codex[file.name] = CODEX_CONFIG.buildSuccessState({
      ...
      if (provider === 'xai') {
        const summaries = successful
          .map((entry) => buildXaiBillingSummary(parseXaiBillingPayload(entry.body)?.config))
          .filter((value): value is XaiBillingSummary => value !== null);
        const summary = summaries.reduce<XaiBillingSummary | null>(
          (current, item) => (current ? mergeXaiBillingSummaries(current, item) : item),
          null
        );
        if (!summary) throw new Error(firstError?.body || t('xai_quota.empty_data'));
        result.xai[file.name] = XAI_CONFIG.buildSuccessState(summary);
        return;
      }

      if (provider === 'kimi') {
        const usage = successful.find((entry) => entry.endpoint === 'usage');
        const payload = usage ? parseKimiUsagePayload(usage.body) : null;
        if (!payload) throw new Error(firstError?.body || t('kimi_quota.empty_data'));
        result.kimi[file.name] = KIMI_CONFIG.buildSuccessState(buildKimiQuotaRows(payload));
        return;
      }

      if (provider === 'claude') {
        const usage = successful.find((entry) => entry.endpoint === 'usage');
        const payload = usage ? parseClaudeUsagePayload(usage.body) : null;
        if (!payload) throw new Error(firstError?.body || t('claude_quota.empty_windows'));
        result.claude[file.name] = CLAUDE_CONFIG.buildSuccessState({
      ...
      if (provider === 'codex') result.codex[file.name] = CODEX_CONFIG.buildErrorState(message, status);
      if (provider === 'xai') result.xai[file.name] = XAI_CONFIG.buildErrorState(message, status);
      if (provider === 'kimi') result.kimi[file.name] = KIMI_CONFIG.buildErrorState(message, status);
      if (provider === 'claude') result.claude[file.name] = CLAUDE_CONFIG.buildErrorState(message, status);
    }
  });
  return result;
}
