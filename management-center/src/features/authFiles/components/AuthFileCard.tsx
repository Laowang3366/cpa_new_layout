...
export function AuthFileCard(props: AuthFileCardProps) {
  ...
  const quotaStatus = useQuotaStore((state) => {
    if (resolvedQuotaType === 'antigravity') return state.antigravityQuota[file.name]?.status;
    if (resolvedQuotaType === 'claude') return state.claudeQuota[file.name]?.status;
    if (resolvedQuotaType === 'codex') return state.codexQuota[file.name]?.status;
    if (resolvedQuotaType === 'kimi') return state.kimiQuota[file.name]?.status;
    if (resolvedQuotaType === 'xai') return state.xaiQuota[file.name]?.status;
    return undefined;
  });
  ...
  return (
    <div
      className={`${styles.fileCard} ${compact ? styles.fileCardCompact : ''} ${listMode ? styles.fileCardList : ''} ${providerCardClass} ${selected ? styles.fileCardSelected : ''} ${file.disabled ? styles.fileCardDisabled : ''}`}
    >
                ...
                <div className={styles.cardUtilityActions}>
                  {resolvedQuotaType && (
                    <Button
                      ...
                      disabled={disableControls || file.disabled || quotaStatus === 'loading'}
                    >
                      {quotaStatus === 'loading' ? (
