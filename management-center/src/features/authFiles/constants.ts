...
export const AUTH_FILE_PAGE_SIZES = [10, 30, 50] as const;
export const MIN_CARD_PAGE_SIZE = AUTH_FILE_PAGE_SIZES[0];
export const MAX_CARD_PAGE_SIZE = AUTH_FILE_PAGE_SIZES[AUTH_FILE_PAGE_SIZES.length - 1];
...
export const HEALTHY_STATUS_MESSAGES = new Set(['ok', 'healthy', 'ready', 'success', 'available']);
...
export const clampCardPageSize = (value: number) => {
  const normalized = Math.min(MAX_CARD_PAGE_SIZE, Math.max(MIN_CARD_PAGE_SIZE, Math.round(value)));
  return AUTH_FILE_PAGE_SIZES.reduce((closest, candidate) =>
...
export const getAuthFileModifiedTimestamp = (item: AuthFileItem): number => {
