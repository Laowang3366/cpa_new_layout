...
import { authFilesApi, quotaGuardianApi } from '@/services/api';
import {
  QuotaSection,
  ANTIGRAVITY_CONFIG,
  CLAUDE_CONFIG,
  ...
  const loadFiles = useCallback(async () => {
    ...
    try {
      const [data, snapshotData] = await Promise.all([
        authFilesApi.list(),
        quotaGuardianApi.snapshots().catch(() => ({ snapshots: [] })),
      ]);
      const nextFiles = data?.files || [];
      setFiles(nextFiles);
      const hydrated = hydrateGuardianSnapshots(nextFiles, snapshotData.snapshots, t);
